...
1. **Block `distr_failed` / `accum_failed` on climactic break bars.** A climactic up-bar inside a distribution range is more consistent with a late-stage BC / final blow-off than with a clean re-accumulation breakout. If any bar within `CLIMACTIC_BREAK_BLOCK_LOOKBACK = 7` bars of the range-fail trigger has climactic up-bar shape, the failure event is suppressed — wait for the breakout to mature into non-climactic continuation before declaring `distr_failed`. Mirror logic applies to `accum_failed` on climactic down-bars. The 7-bar lookback (vs the original 3) covers the typical span from a climactic blow-off to the bars that finally close above the range; on SCMA this delayed a misfiring `distr_failed` from bar 178 to bar 214, eliminating a spurious BC/AR_down pair that fired during the false re-MARKUP.
